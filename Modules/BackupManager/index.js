...
Manager.ImportConfig = async (Path) => {
    ...
    try {
        ...
        for (const { UUID, Nickname, Hostname, MacAddress, GroupID, Weight, Version, IP, Timestamp } of ImportedConfig.Clients) {
            Logger.log(`Importing client: [${UUID}] ${Nickname} [${Hostname}]`);
            await DB.Run('INSERT OR REPLACE INTO Clients (UUID, Nickname, Hostname, MacAddress, GroupID, Weight, Version, IP, Timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
                UUID, 
                Nickname, 
                Hostname, 
                MacAddress, 
