...
document.addEventListener("keydown", function (e) {
	...
	if ((e.ctrlKey || e.metaKey) && !e.shiftKey && !e.altKey && !e.repeat && e.key.toLowerCase() === "q") {
		e.preventDefault();
		try {
...
window.API.UpdateScriptExecutions(async (Executions) => {
	...
	const names = Array.from(
		new Set(
			Executions.map((r) => (r && r.Script && r.Script.Name ? String(r.Script.Name).trim() : null)).filter(
				Boolean
			)
		)
	);
...
// Build and show the Keyboard Shortcuts modal
	function ShowShortcutsModal() {
		try { $("#KEYBOARD_SHORTCUTS_LIST").html(""); } catch {}
		const items = [];
	...
	const formatShortcut = (text) => {
		...
		const html = parts.map(part => {
			if (part.includes('/')) {
				return part.split('/').map(p => `<kbd>${Safe(p.trim())}</kbd>`).join('<span class="key-sep">/</span>');
			}
...
function AddAlert({ type = "info", severity = "info", title = "", message = "", clientUUID = null, iconHtml = null }) {
	const alert = {
...
async function Notify(Message, Type = "info", Duration = 5000) {
	showAlertStyleToast({ title: Message, message: "", type: Type, duration: Duration, linkAlert: false });
}
