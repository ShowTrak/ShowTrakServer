...
export const AUDIO_ASSET_MAX_DURATION_SECONDS = 15;

/** One in-flight (or preview) audio playback tracked in {@link ActiveAudioPlaybacks}. */
interface AudioPlaybackEntry {
...
/** Controller for the currently-previewing asset. */
...
export let ActiveAudioPlaybacks: AudioPlaybackEntry[] = [];
export let AudioPlaybackContext: AudioContext | null = null;
export let ActivePreviewPlayback: PreviewPlayback | null = null;

export function getAudioPlaybackContext() {
...
export function toVisualVolume(BackendVolume: unknown) {
...
export function setPreviewButtonState(ID: string, IsPlaying: boolean) {
...
export function AudioAssetByID(ID: string) {
...
export async function SelectAndImportAudioAssets() {
  let Candidates: AudioAssetInspection[];
  try {
  ...
  for (const Candidate of Candidates) {
    ...
    let Duration: number;
    try {
      Duration = await getAudioDurationFromDataURL(String(Candidate.DataURL));
    } catch (DurErr) {
      Failures.push(`${Name}: ${ErrorMessage(DurErr, 'Invalid audio file')}`);
      continue;
    }
    ...
    try {
      ...
      Failures.push(`${Name}: ${ErrorMessage(ImportThrow, 'Import failed')}`);
    }
  }
...
export async function PreviewAudioAsset(ID: string) {
...
export function formatAudioAssetSize(Bytes: number) {
...
export function RenderAudioAssetsList() {
...
export function BindAudioAssetRowHandlers() {
  const $host = $('#AUDIO_ASSETS_LIST');
  ...
  $host
    ...
    .on('change', async function () {
      const ID = $(this).closest('[data-audio-id]').attr('data-audio-id');
      if (!ID) return;
      const VisualVolume = parseInt(String($(this).val() || ''), 10);
...
export async function OpenAudioAssetsManager() {
  await LoadAudioAssets();
  RenderAudioAssetsList();
