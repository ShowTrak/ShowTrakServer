...
(function () {
  ...
  type FlipState = Map<string, DOMRect>;
  ...
  function CaptureFlipState(root: Element | null): FlipState {
    const state: FlipState = new Map();
  ...
  function FindFlipAncestorRecord(
    node: HTMLElement,
    recordByNode: Map<HTMLElement, FlipRecord>
  ): FlipRecord | null {
  ...
  function PlayFlipState(root: Element | null, previous: unknown) {
    if (!root || !AnimationsEnabled() || !(previous instanceof Map) || previous.size === 0) return;
    const previousRects: FlipState = previous;

    const nodes = Array.from(root.querySelectorAll<HTMLElement>(FLIP_SELECTOR));
    const records: FlipRecord[] = [];
    const recordByNode = new Map<HTMLElement, FlipRecord>();
  ...
  function AnimateMove(node: HTMLElement, dx: number, dy: number) {
  ...
  function AnimateReflow(root: Element | null, renderFn: () => void) {
