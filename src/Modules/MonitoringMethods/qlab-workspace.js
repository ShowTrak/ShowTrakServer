...
function SlipDecodeFrames(Buffer0) {
  const Frames = [];
  let Current = [];
  let Escaped = false;
...
function EncodeOscMessageTcp(Address) {
...
function ReadOscMessageStrings(Packet) {
  const Strings = [];
  try {
...
function TryParseWorkspaceData(Str) {
...
function ExtractWorkspaceData(Buffer0) {
  const Candidates = [];
...
function WorkspaceMatches(Workspace, Wanted) {
...
async function QueryWorkspaces(Address, Port, TimeoutMs) {
...
async function Run(Target) {
  ...
  if (!QueryResult || !QueryResult.Success) {
    return {
      Success: false,
      Error: (QueryResult && QueryResult.Error) || 'No reply from QLab',
    };
  }
...
function Debug(Result, Target) {
  ...
  const Workspaces = Array.isArray(Result && Result.Workspaces) ? Result.Workspaces : [];
