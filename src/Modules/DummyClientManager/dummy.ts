...
import { ClampInterval } from './normalize';
...
// Fields required to construct a DummyClient. Mirrors DummyClientRow, but IP is
// optional so freshly-created dummies (which have no recorded heartbeat IP yet)
// can be built without one.
export interface DummyClientInit {
  UUID: string;
  DummyID: string;
...
// Snapshot shape produced by ToJSON(). A structurally looser sibling of the
// shared DummyClientView (State/Version/Type are plain strings here because the
// runtime class stores them untyped).
export interface DummyClientSnapshot {
  UUID: string;
  DummyID: string;
  Nickname: string;
  Hostname: string;
...
class DummyClient {
  ...
  constructor(Row: DummyClientInit) {
    this.UUID = Row.UUID;
  ...
  // Force the dummy back to its freshly-loaded Idle state, discarding any live
  // connection state. Stops the watchdog; the dummy will re-arm and go Online
  // again the next time a heartbeat is received. Mirrors the constructor's
  // runtime defaults.
  ResetToIdle() {
    this._clearTimer();
    this.MissedCount = 0;
