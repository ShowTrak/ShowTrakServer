...
export = function registerSystemValidators(Manager: IPCValidationManager): void {
  Manager.NetworkDiscoveryScanID = (value: unknown) => {
    return normalizeNonEmptyString(value, 'ScanID', { minLength: 8, maxLength: 128 });
  };

  Manager.NetworkDiscoveryScanOptions = (value: unknown) => {
    ...
    if (Object.prototype.hasOwnProperty.call(value, 'ProbePorts')) {
      ...
      const ports = value.ProbePorts.map((p: unknown) => {
        const port = Number(p);
  ...
  Manager.SettingUpdatePayload = (key: unknown, value: unknown) => {
